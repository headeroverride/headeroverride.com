...
export const metadata: Metadata = {
  ...
  openGraph: { title: `${title} | Header Override`, description, url: pageUrl, siteName: "Header Override", type: "article", publishedTime: guideDatePublished, modifiedTime: guideDateModified, images: [{ url: "/screenshots/guide-staging-profiles.png", width: 760, height: 520, alt: "Header Override profiles menu" }] },
  twitter: { card: "summary_large_image", title: `${title} | Header Override`, description, images: ["/screenshots/guide-staging-profiles.png"] }
};

const articleJsonLd = { "@context": "https://schema.org", "@type": "TechArticle", headline: title, description, datePublished: guideDatePublished, dateModified: guideDateModified, url: pageUrl, image: "https://headeroverride.com/screenshots/guide-staging-profiles.png", author: { "@type": "Organization", name: "Header Override", url: "https://headeroverride.com" }, publisher: { "@type": "Organization", name: "Header Override", logo: { "@type": "ImageObject", url: "https://headeroverride.com/icons/icon-128.png" } } };
...
export default function DebugWithHeaderProfilesPage() {
  return <main className="legal-page guide-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([articleJsonLd, breadcrumbJsonLd]) }} /><GuideNav /><article className="guide-article shell"><GuideArticleHeader category="Authentication & Cookies" description={description} pageUrl={pageUrl} readTime="5 min read" title={title} /><div className="guide-layout"><aside className="guide-aside"><p className="guide-aside-label">On this page</p><a href="#why-profiles">Why use profiles</a><a href="#workflow">A repeatable workflow</a><a href="#screenshot">Set up profiles</a></aside><div className="guide-content">
